...
vector<vector<RWLock> > tablero_locks;  // tiene los locks para acceder a cada casillero

unsigned int ancho = -1;
unsigned int alto = -1;

// Funciones para leer y escribir en los tableros USANDO LOCKS POR CASILLERO
char leerCasillero(vector<vector<char> > &tablero, int f, int c){
        tablero_locks[f][c].rlock();
        char res = tablero[f][c];
        tablero_locks[f][c].runlock();
        return res;
}
void escribirCasillero(vector<vector<char> > &tablero, int f, int c, char valor){
        tablero_locks[f][c].wlock();
        tablero[f][c] = valor;
        tablero_locks[f][c].wunlock();
        return;
}
...
int main(int argc, const char* argv[]) {
    ...
    tablero_locks = vector<vector<RWLock> >(alto);
    for (unsigned int i = 0; i < alto; ++i) {
        tablero_locks[i] = vector<RWLock>(ancho, RWLock());
    }

    int socket_size;
    struct sockaddr_in local, remoto;
    int socketfd_clientes[MAX_JUGADORES];
    ...
    // escuchar en el socket hasta MAX_JUGADORES conexiones entrantes
    if (listen(socket_servidor, MAX_JUGADORES) == -1) {
    ...
    for(int cur = 0; cur < MAX_JUGADORES; cur++){
            while (true) {
                if ((socketfd_clientes[cur] = accept(socket_servidor, (struct sockaddr*) &remoto, (socklen_t*) &socket_size)) == -1)
                    cerr << "Error al aceptar conexion" << endl;
                else {
                    pthread_create(&(thread[cur]), NULL, &atendedor_de_jugador, (void*)(&(socketfd_clientes[cur])));
                    break;
                }
            }
        }
...
void *atendedor_de_jugador(void *socket_fd_ptr) {
    ...
    if (recibir_nombre(socket_fd, nombre_jugador) != 0) {
        ...
        char *nombre_invalido = (char*)("nombre_invalido");
        terminar_servidor_de_jugador(socket_fd, palabra_actual, nombre_invalido);
    }

    if (enviar_dimensiones(socket_fd) != 0) {
        // se produjo un error al enviar. Cerramos todo.
        terminar_servidor_de_jugador(socket_fd, palabra_actual, nombre_jugador);
    }
    ...
    while (true) {
        ...
        if (comando == MSG_LETRA) {
            ...
            if (es_ficha_valida_en_palabra(ficha, palabra_actual)) {
                ...
                escribirCasillero(tablero_letras, ficha.fila, ficha.columna, ficha.letra);
                // OK
            ...
            for (list<Casillero>::const_iterator casillero = palabra_actual.begin(); casillero != palabra_actual.end(); casillero++) {
                escribirCasillero(tablero_palabras, casillero->fila, casillero->columna, casillero->letra);
...
int enviar_tablero(int socket_fd) {
    ...
    for (unsigned int fila = 0; fila < alto; ++fila) {
        for (unsigned int col = 0; col < ancho; ++col) {
            char letra = leerCasillero(tablero_palabras, fila, col);
...
void terminar_servidor_de_jugador(int socket_fd, list<Casillero>& palabra_actual, char *nombre) {
    cout << "Se interrumpió la comunicación con el jugador " << nombre << " :(" << endl;
...
void quitar_letras(list<Casillero>& palabra_actual) {
    for (list<Casillero>::const_iterator casillero = palabra_actual.begin(); casillero != palabra_actual.end(); casillero++) {
        escribirCasillero(tablero_letras, casillero->fila, casillero->columna, VACIO);
    }
    palabra_actual.clear();
}
...
bool es_ficha_valida_en_palabra(const Casillero& ficha, const list<Casillero>& palabra_actual) {
    ...
    if (leerCasillero(tablero_letras, ficha.fila, ficha.columna) != VACIO) {
        return false;
    }
    ...
    if (palabra_actual.size() > 0) {
        ...
        if (distancia_vertical == 0) {
            ...
            for (unsigned int columna = mas_distante.columna; columna != ficha.columna; columna += paso) {
                ...
                if (!(puso_letra_en(ficha.fila, columna, palabra_actual)) && leerCasillero(tablero_palabras, ficha.fila, ficha.columna) == VACIO) {
                    return false;
                }
            }
            ...
            for (unsigned int fila = mas_distante.fila; fila != ficha.fila; fila += paso) {
                ...
                if (!(puso_letra_en(fila, ficha.columna, palabra_actual)) && leerCasillero(tablero_palabras, fila, ficha.columna) == VACIO) {
                    return false;
                }
            }
        }
