...
unsigned int Ext2FS::get_block_size(){
	return 1024 << (_superblock->log_block_size);
}

unsigned int Ext2FS::indirect_once(unsigned int lba, unsigned int index)
{
	unsigned char buf[get_block_size()];
	read_block(lba, (unsigned char*)(buf));
	return ((unsigned int*)(buf))[index];
}
...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	Ext2FSBlockGroupDescriptor * gd = block_group(group);
	
	unsigned int inodes_per_block = block_size / _superblock->inode_size;
	
	unsigned char * buff = new unsigned char[block_size];
	unsigned int lba = gd->inode_table + index / inodes_per_block;
	
	read_block(lba, buff);
	
	struct Ext2FSInode* res = new struct Ext2FSInode;
	//memcpy(res, (Ext2FSInode*)(buff)[index % inodes_per_block], _superblock->inode_size);
	unsigned int resto = index % inodes_per_block;
	
	memcpy(res, &buff[_superblock->inode_size*resto], _superblock->inode_size);
...
{
	unsigned int lba;
	if(block_number<directOffset)
	{
		lba = inode->block[block_number];
	}
	else if(block_number< fstIndOffset)
	{
		lba = inode->block[12];
		block_number -= directOffset;
		lba = indirect_once(lba, block_number);
	}
	else if(block_number< sndIndOffset)
	{
		lba = inode->block[13];
		block_number -= fstIndOffset;
		
		lba = indirect_once(lba, block_number / fstIndSize);
		block_number %= fstIndSize;
	...
	{
		lba = inode->block[14];
		block_number -= sndIndOffset;

		lba = indirect_once(lba, block_number / sndIndSize);
		block_number %= sndIndSize;
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	unsigned int tot_blocks = from->blocks;
	unsigned int block_size = get_block_size();
	unsigned char dir_data[tot_blocks * block_size];
	
	std::cerr << "Comienza a copiar (total=" << tot_blocks << ")...";
	// This copies the entire dir data to dir_data
	for(int i=0;i<tot_blocks;i++){
		unsigned char block[block_size];
		unsigned int lba = get_block_address(from, i);
		
		//std::cerr << lba << std::endl;
		read_block(lba, (unsigned char*)(block));
		memcpy(dir_data + i * block_size, block, block_size);
	}
	std::cerr << " lesto.\n";
	
	std::cerr << "Empiezo a recorrer...\n";
	int idx = 0;
	while(idx < from->size){
		Ext2FSDirEntry * dir_entry = (Ext2FSDirEntry *)(dir_data + idx);
		
		if(strncmp(filename, dir_entry->name, dir_entry->name_length) == 0){
			std::cerr << "Encontre lo que buscaba. El inodo " << dir_entry->inode << "\n";
			return load_inode(dir_entry->inode);
		}
		idx += dir_entry->record_length;
		//std::cerr << "Datos de la dir_entry: (" << dir_entry->inode << ", " << dir_entry->record_length << ", " << dir_entry->name_length << ", " << dir_entry->file_type << ", " << dir_entry->name << std::endl;
	}
	//std::cerr << "No encontre nada.\n";
